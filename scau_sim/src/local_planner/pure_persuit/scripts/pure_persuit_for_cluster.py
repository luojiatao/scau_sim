...
class PurePersuit:
	def __init__(self):
		...
		rospy.Subscriber('/goal', PoseStamped, self.path_cb, queue_size = 1)
	...
	def path_cb(self,plan_msg):
		# plan_msg 是 PoseStamped 类型，直接获取位姿数据
		target_pose = plan_msg.pose

		# 检查是否已经到达目标点
		if not self.target_reached:
			# 设置当前路径点为接收到的目标位姿
			self.currentpath = target_pose
			rospy.logdebug('Already get PoseStamped~~~~')
